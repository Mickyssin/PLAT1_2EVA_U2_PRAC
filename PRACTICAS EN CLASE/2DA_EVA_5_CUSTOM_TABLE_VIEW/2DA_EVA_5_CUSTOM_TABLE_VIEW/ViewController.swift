...
//
//  Created by LUIS FERNANDO on 16/03/17.
...
class ViewController: UIViewController {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell { //cellforRow reutiliza las celdas
        ...
        let celda = tableView.dequeueReusableCellWithIdentifier("primera") as! MyTable
    ...
    func comparteRest(action: UITableViewRowAction, indexPath: NSIndexPath) -> Void {
        ...
        let miImagen = UIImage(named: misDatos[indexPath.row]) //Selecciona el arreglo de imagenes y elige en el que se dio click
